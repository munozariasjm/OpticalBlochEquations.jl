...
function plot_survival_trajectories(results, direction)
    max_t_id = 1
    plot_ts = Float64[]
   for i in 1:length(results.times)
        if length(results.times[i]) > max_t_id
   ...
   if direction == "x"
       plot(legend=false, title="x trajectory", xlabel="time (ms)",ylabel="x (mm)")
       for j in 1:length(results.times)
          if length(results.times[j]) < max_t_id 
               continue
          end                                                                                                      
   ...
   elseif direction == "y"
       plot(legend=false, title="y trajectory", xlabel="time (ms)",ylabel="y (mm)")
       for j in 1:length(results.times)
          if length(results.times[j]) < max_t_id 
               continue
          end                                                                                                             
   ...
   elseif direction == "z"
       plot(legend=false, title="z trajectory", xlabel="time (ms)",ylabel="z (mm)")
       for j in 1:length(results.times)
          if length(results.times[j]) < max_t_id 
               continue
          end                                                                                                                      
   ...
   elseif direction == "all"
       plot(legend=false, title="distance from centre", xlabel="time (ms)",ylabel="distance (mm)")
       for j in 1:length(results.times)
          if length(results.times[j] )< max_t_id
               continue
          end                                                                                                                              
