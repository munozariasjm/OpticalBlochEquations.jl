...
function get_average_diffusion_from_record(results,run_time,count_last_scatters,rmax)
    id_survived = survived_t(results, run_time * 1e3; rmax=rmax)
    diffusion_sum = 0
    for i in id_survived
        diffusion_sum += mean(results.diffusion_records[i][end-count_last_scatters:end])
    end
    return diffusion_sum / length(id_survived)
end


                                                                                
function find_diffusion_constant(params; run_time=5e-3, n_particles=20, ramp_time=1e-6, temp=1e-4, diameter=80e-6, count_last_scatters=1000, rmax=10.0)
    ...
    diffusion_constant = get_average_diffusion_from_record(results, run_time, count_last_scatters, rmax)
