...
function define_lasers(
    ...
    ω2 = 2π * (energy(states[end]) - energy(states[6])) + Δ2 * 1e6 * 2π
    ω3 = 2π * (energy(states[end]) - energy(states[6])) + Δ3 * 1e6 * 2π
    ω4 = 2π * (energy(states[end]) - energy(states[10])) + Δ4 * 1e6 * 2π
    ω5 = 2π * (energy(states[end]) - energy(states[10])) + Δ4 * 1e6 * 2π
    ...
    s1z = s1 * (1+s_imbalance[3]*sz_rand)
    k̂ = kx;  ϵ1 = ϕs[1]*rotate_pol(pol1_x, k̂); ϵ_func1 = ϵ_(ϵ1, 1); laser1 = Field(k̂, ϵ_func1, ω1,  s_gaussian_ramp(s1x, s_ramp_to, s_ramp_time, (2,3), (x_center_y, x_center_z)))
    k̂ = -kx; ϵ2 = ϕs[1]*rotate_pol(pol1_x, k̂); ϵ_func2 = ϵ_(ϵ2, 2); laser2 = Field(k̂, ϵ_func2, ω1, s_gaussian_ramp(s1x*(1-retro_loss), s_ramp_to, s_ramp_time,  (2,3), (x_center_y, x_center_z)))
    k̂ = ky;  ϵ3 = ϕs[3]*rotate_pol(pol1_x, k̂); ϵ_func3 = ϵ_(ϵ3, 3); laser3 = Field(k̂, ϵ_func3, ω1,  s_gaussian_ramp(s1y, s_ramp_to, s_ramp_time,  (1,3), (y_center_x, y_center_z)))
    k̂ = -ky; ϵ4 = ϕs[3]*rotate_pol(pol1_x, k̂); ϵ_func4 = ϵ_(ϵ4, 4); laser4 = Field(k̂, ϵ_func4, ω1,  s_gaussian_ramp(s1y*(1-retro_loss), s_ramp_to, s_ramp_time,  (1,3), (y_center_x, y_center_z)))
    k̂ = +kz; ϵ5 = ϕs[5]*rotate_pol(flip(pol1_x), k̂); ϵ_func5 = ϵ_(ϵ5, 5); laser5 = Field(k̂, ϵ_func5, ω1,  s_gaussian_ramp(s1z, s_ramp_to, s_ramp_time,  (1,2), (z_center_x, z_center_y)))
    k̂ = -kz; ϵ6 = ϕs[5]*rotate_pol(flip(pol1_x), k̂); ϵ_func6 = ϵ_(ϵ6, 6); laser6 = Field(k̂, ϵ_func6, ω1, s_gaussian_ramp(s1z*(1-retro_loss), s_ramp_to, s_ramp_time,  (1,2), (z_center_x, z_center_y)))
    ...
    s2z = s2 * (1+s_imbalance[3]*sz_rand)
    k̂ = +kx; ϵ7 = ϕs[1]*rotate_pol(pol2_x, k̂); ϵ_func7 = ϵ_(ϵ7, 1); laser7 = Field(k̂, ϵ_func7, ω2, s_gaussian_ramp(s2x, s_ramp_to, s_ramp_time, (2,3), (x_center_y, x_center_z)))
    k̂ = -kx; ϵ8 = ϕs[1]*rotate_pol(pol2_x, k̂); ϵ_func8 = ϵ_(ϵ8, 2); laser8 = Field(k̂, ϵ_func8, ω2, s_gaussian_ramp(s2x*(1-retro_loss), s_ramp_to, s_ramp_time, (2,3), (x_center_y, x_center_z)))
    k̂ = +ky; ϵ9 = ϕs[3]*rotate_pol(pol2_x, k̂); ϵ_func9 = ϵ_(ϵ9, 3); laser9 = Field(k̂, ϵ_func9, ω2, s_gaussian_ramp(s2y, s_ramp_to, s_ramp_time,  (1,3), (y_center_x, y_center_z)))
    k̂ = -ky; ϵ10 = ϕs[3]*rotate_pol(pol2_x, k̂); ϵ_func10 = ϵ_(ϵ10, 4); laser10 = Field(k̂, ϵ_func10, ω2, s_gaussian_ramp(s2y*(1-retro_loss), s_ramp_to, s_ramp_time,  (1,3), (y_center_x, y_center_z)))
    k̂ = +kz; ϵ11 = ϕs[5]*rotate_pol(flip(pol2_x), k̂); ϵ_func11 = ϵ_(ϵ11, 5); laser11 = Field(k̂, ϵ_func11, ω2, s_gaussian_ramp(s2z, s_ramp_to, s_ramp_time,  (1,2), (z_center_x, z_center_y)))
    k̂ = -kz; ϵ12 = ϕs[5]*rotate_pol(flip(pol2_x), k̂); ϵ_func12 = ϵ_(ϵ12, 6); laser12 = Field(k̂, ϵ_func12, ω2, s_gaussian_ramp(s2z*(1-retro_loss), s_ramp_to, s_ramp_time,  (1,2), (z_center_x, z_center_y)))
    ...
    s3z = s3 * (1+s_imbalance[3]*sz_rand)
    k̂ = +kx; ϵ13 = ϕs[1]*rotate_pol(pol3_x, k̂); ϵ_func13 = ϵ_(ϵ13, 1); laser13 = Field(k̂, ϵ_func13, ω3, s_gaussian_ramp(s3x, s_ramp_to, s_ramp_time, (2,3), (x_center_y, x_center_z)))
    k̂ = -kx; ϵ14 = ϕs[1]*rotate_pol(pol3_x, k̂); ϵ_func14 = ϵ_(ϵ14, 2); laser14 = Field(k̂, ϵ_func14, ω3, s_gaussian_ramp(s3x*(1-retro_loss), s_ramp_to, s_ramp_time, (2,3), (x_center_y, x_center_z)))
    k̂ = +ky; ϵ15 = ϕs[3]*rotate_pol(pol3_x, k̂); ϵ_func15 = ϵ_(ϵ15, 3); laser15 = Field(k̂, ϵ_func15, ω3, s_gaussian_ramp(s3y, s_ramp_to, s_ramp_time,  (1,3), (y_center_x, y_center_z)))
    k̂ = -ky; ϵ16 = ϕs[3]*rotate_pol(pol3_x, k̂); ϵ_func16 = ϵ_(ϵ16, 4); laser16 = Field(k̂, ϵ_func16, ω3, s_gaussian_ramp(s3y*(1-retro_loss), s_ramp_to, s_ramp_time,  (1,3), (y_center_x, y_center_z)))
    k̂ = +kz; ϵ17 = ϕs[5]*rotate_pol(flip(pol3_x), k̂); ϵ_func17 = ϵ_(ϵ17, 5); laser17 = Field(k̂, ϵ_func17, ω3, s_gaussian_ramp(s3z, s_ramp_to, s_ramp_time,  (1,2), (z_center_x, z_center_y)))
    k̂ = -kz; ϵ18 = ϕs[5]*rotate_pol(flip(pol3_x), k̂); ϵ_func18 = ϵ_(ϵ18, 6); laser18 = Field(k̂, ϵ_func18, ω3, s_gaussian_ramp(s3z*(1-retro_loss), s_ramp_to, s_ramp_time,  (1,2), (z_center_x, z_center_y)))
    ...
    k̂ = -kx; ϵ20 = ϕs[1]*rotate_pol(pol4_x, k̂); ϵ_func20 = ϵ_(ϵ20, 2); laser20 = Field(k̂, ϵ_func20, ω4, s_gaussian_ramp(s4x*(1-retro_loss), s_ramp_to, s_ramp_time, (2,3), (x_center_y, x_center_z)))
    ...
    k̂ = -ky; ϵ22 = ϕs[3]*rotate_pol(pol4_x, k̂); ϵ_func22 = ϵ_(ϵ22, 4); laser22 = Field(k̂, ϵ_func22, ω4, s_gaussian_ramp(s4y*(1-retro_loss), s_ramp_to, s_ramp_time,  (1,3), (y_center_x, y_center_z)))
    ...
    k̂ = -kz; ϵ24 = ϕs[5]*rotate_pol(flip(pol4_x), k̂); ϵ_func24 = ϵ_(ϵ24, 6); laser24 = Field(k̂, ϵ_func24, ω4, s_gaussian_ramp(s4z*(1-retro_loss), s_ramp_to, s_ramp_time,  (1,2), (z_center_x, z_center_y)))
    ...
    k̂ = +kx; ϵ25 = ϕs[1]*rotate_pol(pol5_x, k̂); ϵ_func25 = ϵ_(ϵ25, 1); laser25 = Field(k̂, ϵ_func25, ω5, s_gaussian_ramp(s5x, s_ramp_to, s_ramp_time, (2,3), (x_center_y, x_center_z)))
    k̂ = -kx; ϵ26 = ϕs[1]*rotate_pol(pol5_x, k̂); ϵ_func26 = ϵ_(ϵ26, 2); laser26 = Field(k̂, ϵ_func26, ω5, s_gaussian_ramp(s5x*(1-retro_loss), s_ramp_to, s_ramp_time, (2,3), (x_center_y, x_center_z)))
    k̂ = +ky; ϵ27 = ϕs[3]*rotate_pol(pol5_x, k̂); ϵ_func27 = ϵ_(ϵ27, 3); laser27 = Field(k̂, ϵ_func27, ω5, s_gaussian_ramp(s5y, s_ramp_to, s_ramp_time,  (1,3), (y_center_x, y_center_z)))
    k̂ = -ky; ϵ28 = ϕs[3]*rotate_pol(pol5_x, k̂); ϵ_func28 = ϵ_(ϵ28, 4); laser28 = Field(k̂, ϵ_func28, ω5, s_gaussian_ramp(s5y*(1-retro_loss), s_ramp_to, s_ramp_time,  (1,3), (y_center_x, y_center_z)))
    k̂ = +kz; ϵ29 = ϕs[5]*rotate_pol(flip(pol5_x), k̂); ϵ_func29 = ϵ_(ϵ29, 5); laser29 = Field(k̂, ϵ_func29, ω5, s_gaussian_ramp(s5z, s_ramp_to, s_ramp_time,  (1,2), (z_center_x, z_center_y)))
    k̂ = -kz; ϵ30 = ϕs[5]*rotate_pol(flip(pol5_x), k̂); ϵ_func30 = ϵ_(ϵ30, 6); laser30 = Field(k̂, ϵ_func30, ω5, s_gaussian_ramp(s5z*(1-retro_loss), s_ramp_to, s_ramp_time,  (1,2), (z_center_x, z_center_y)))
