...
function define_lasers(
    ...
    ω2 = 2π * (energy(states[end]) - energy(states[1])) + Δ2 * 1e6 * 2π
    ω3 = 2π * (energy(states[end]) - energy(states[6])) + Δ3 * 1e6 * 2π
    ω4 = 2π * (energy(states[end]) - energy(states[6])) + Δ4 * 1e6 * 2π
    ω5 = 2π * (energy(states[end]) - energy(states[6])) + Δ4 * 1e6 * 2π
    ...
    k̂ = -kx; ϵ26 = ϕs[2]*rotate_pol(pol5_x, k̂); ϵ_func26 = ϵ_(ϵ26, 2); laser26 = Field(k̂, ϵ_func26, ω5, s_gaussian_ramp(s5x*(1-retro_loss), s_ramp_to, s_ramp_time, (2,3), (x_center_y, x_center_z)))
    ...
    k̂ = -ky; ϵ28 = ϕs[4]*rotate_pol(pol5_x, k̂); ϵ_func28 = ϵ_(ϵ28, 4); laser28 = Field(k̂, ϵ_func28, ω5, s_gaussian_ramp(s5y*(1-retro_loss), s_ramp_to, s_ramp_time,  (1,3), (y_center_x, y_center_z)))
    ...
    k̂ = -kz; ϵ30 = ϕs[6]*rotate_pol(flip(pol5_x), k̂); ϵ_func30 = ϵ_(ϵ30, 6); laser30 = Field(k̂, ϵ_func30, ω5, s_gaussian_ramp(s5z*(1-retro_loss), s_ramp_to, s_ramp_time,  (1,2), (z_center_x, z_center_y)))
