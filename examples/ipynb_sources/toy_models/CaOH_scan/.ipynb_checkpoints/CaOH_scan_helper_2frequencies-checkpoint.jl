...
function init_MOT_distribution(T, diameter)
    kB = 1.381e-23
    m = @with_unit 57 "u"
    σ = sqrt(kB * T / m)
    
    r = Normal(0.0, diameter)
    v = Normal(0.0, σ)
    return r, v
end
...
function gaussian_intensity_along_axes(r, axes)
    ...
    exp(-2*d2/(5.5e-3/(1/k))^2)
end

function make_problem_with_param(molecule_package, param)    
    t_end = param.t_end
    pol1_x, pol2_x = param.pol1_x, param.pol2_x

    s1, s2 = param.s1, param.s2
    Δ1, Δ2 = param.Δ1, param.Δ2
    B_gradient = param.B_gradient
    temp = param.temp
    diameter = param.diameter
    ramp_time = param.ramp_time
    
    x_dist, vx_dist = init_MOT_distribution(temp, diameter)
    y_dist, vy_dist = init_MOT_distribution(temp, diameter)
    z_dist, vz_dist = init_MOT_distribution(temp, diameter)
    ...
    ϵ_(ϵ, f) = t -> ϵ
    s_gaussian(s,axes) = (r,t) -> s * gaussian_intensity_along_axes(r, axes)
    
    ϕs = [exp(im*2π*rand()),exp(im*2π*rand()),exp(im*2π*rand()),exp(im*2π*rand()),exp(im*2π*rand()),exp(im*2π*rand())]

    k̂ = +x̂; ϵ1 = ϕs[1]*rotate_pol(pol1_x, k̂); ϵ_func1 = ϵ_(ϵ1, 1); laser1 = Field(k̂, ϵ_func1, ω1, s_gaussian(s1, (2,3)))
    k̂ = -x̂; ϵ2 = ϕs[2]*rotate_pol(pol1_x, k̂); ϵ_func2 = ϵ_(ϵ2, 2); laser2 = Field(k̂, ϵ_func2, ω1, s_gaussian(s1, (2,3)))
    k̂ = +ŷ; ϵ3 = ϕs[3]*rotate_pol(pol1_x, k̂); ϵ_func3 = ϵ_(ϵ3, 3); laser3 = Field(k̂, ϵ_func3, ω1, s_gaussian(s1, (1,3)))
    k̂ = -ŷ; ϵ4 = ϕs[4]*rotate_pol(pol1_x, k̂); ϵ_func4 = ϵ_(ϵ4, 4); laser4 = Field(k̂, ϵ_func4, ω1, s_gaussian(s1, (1,3)))
    k̂ = +ẑ; ϵ5 = ϕs[5]*rotate_pol(flip(pol1_x), k̂); ϵ_func5 = ϵ_(ϵ5, 5); laser5 = Field(k̂, ϵ_func5, ω1, s_gaussian(s1, (1,2)))
    k̂ = -ẑ; ϵ6 = ϕs[6]*rotate_pol(flip(pol1_x), k̂); ϵ_func6 = ϵ_(ϵ6, 6); laser6 = Field(k̂, ϵ_func6, ω1, s_gaussian(s1, (1,2)))

    lasers_1 = [laser1, laser2, laser3, laser4, laser5, laser6]

    k̂ = +x̂; ϵ7 = ϕs[1]*rotate_pol(pol2_x, k̂); ϵ_func7 = ϵ_(ϵ7, 1); laser7 = Field(k̂, ϵ_func7, ω2, s_gaussian(s2, (2,3)))
    k̂ = -x̂; ϵ8 = ϕs[2]*rotate_pol(pol2_x, k̂); ϵ_func8 = ϵ_(ϵ8, 2); laser8 = Field(k̂, ϵ_func8, ω2, s_gaussian(s2, (2,3)))
    k̂ = +ŷ; ϵ9 = ϕs[3]*rotate_pol(pol2_x, k̂); ϵ_func9 = ϵ_(ϵ9, 3); laser9 = Field(k̂, ϵ_func9, ω2, s_gaussian(s2, (1,3)))
    k̂ = -ŷ; ϵ10 = ϕs[4]*rotate_pol(pol2_x, k̂); ϵ_func10 = ϵ_(ϵ10, 4); laser10 = Field(k̂, ϵ_func10, ω2, s_gaussian(s2, (1,3)))
    k̂ = +ẑ; ϵ11 = ϕs[5]*rotate_pol(flip(pol2_x), k̂); ϵ_func11 = ϵ_(ϵ11, 5); laser11 = Field(k̂, ϵ_func11, ω2, s_gaussian(s2, (1,2)))
    k̂ = -ẑ; ϵ12 = ϕs[6]*rotate_pol(flip(pol2_x), k̂); ϵ_func12 = ϵ_(ϵ12, 6); laser12 = Field(k̂, ϵ_func12, ω2, s_gaussian(s2, (1,2)))
    ...
    prob = ODEProblem(ψ_stochastic!, p.ψ, t_span, p, callback=cb)
...
function simulate_particles(package, params)
