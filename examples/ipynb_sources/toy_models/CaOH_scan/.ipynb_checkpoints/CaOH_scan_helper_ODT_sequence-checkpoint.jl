...
function make_problem_with_param(molecule_package, package_A, param; variable_budget=false, randomize_initial_vector = true, ψ0=nothing)       t_start = param.t_start
    t_end = param.t_end
...
function continue_particle!(j, results, prob_copy)
    ...
    sol = DifferentialEquations.solve(prob_copy, alg=DP5(), reltol=1e-3, callback=cb, saveat=4000, maxiters=50000000)
