...
""" Update from CaOH_scan_helper_repump: added ODT Hamiltonian, added conservative force, removed repumpers.  """
...
function update_H(H, p, r, τ)
    ...
    H_ODT = p.extra_p.H_ODT_static
    ODT_size = p.extra_p.ODT_size .* p.k
    scalar1 = exp(-2* r[1]^2/ODT_size[1]^2) * exp(-2* r[2]^2/ODT_size[2]^2) * exp(-2* r[3]^2/ODT_size[3]^2)
    @turbo for i in eachindex(H)
        H.re[i] += H_ODT.re[i] * scalar1
        H.im[i] += H_ODT.im[i] * scalar1
    end
...
function make_problem_with_param(molecule_package, package_A, param; variable_budget=false)    
    t_end = param.t_end
    ...
    t_laser_off = param.t_laser_off * Γ
    ...
    extra_p = MutableNamedTuple(
        ...
        H_ODT_copy = StructArray(get_ODT_Hamiltonian_matrix(package, package_A, ODT_intensity, [0,1.0,0]))
        )
    ...
    s_gaussian_ramp(s, factor, ramp_time, axes, centers) = (r,t) -> ((s*factor-s)/ramp_time * min(t, ramp_time) + s) * gaussian_intensity_along_axes(r, axes, centers) * (t < t_laser_off)
...
function simulate_particles_ODT(package, package_A, params; variable_budget=false)
    ...
    Threads.@threads for i ∈ 1:n_threads
        ...
        for j ∈ batch_start_idx:(batch_start_idx + _batch_size - 1) 
            ...
            prob_copy = make_problem_with_param(package, package_A, params, variable_budget=variable_budget)
            cb_copy = deepcopy(cb)
