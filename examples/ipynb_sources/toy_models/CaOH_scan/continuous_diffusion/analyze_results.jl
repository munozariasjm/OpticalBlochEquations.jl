...
function bootstrap_size_fit(results, t; iter = 10, n_values=nothing, range=0.5, n_grids=40)
    sizes = []
...
function bootstrap_temperature_fit(results, t; iter=10, n_values=nothing, range=0.5, n_grids=20)
    temperatures = []
    dt = results.times[1][2]-results.times[1][1]
    t_j = Int(floor(t ÷ dt))                                                            
    x_grid = collect(LinRange(-range, range, n_grids))
