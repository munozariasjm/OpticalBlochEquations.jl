...
function continue_particle_constant_diffusion!(j, results, prob_copy)
    ...
    sol = DifferentialEquations.solve(prob_copy, alg=Euler(), reltol=1e-3, callback=cb, saveat=4000, maxiters=5000000000, dt=prob_copy.p.dt)
...
function make_problem_with_param_constant_diffusion(molecule_package, package_A, param; randomize_initial_vector = true, ψ0=nothing)       
   ...
   photon_budget = param.photon_budget
    ...
    prob = ODEProblem(ψ_stochastic_continuous_diffusion!, p.ψ, t_span, p)
