...
#     println()
    ...
    p.sum_diffusion_x += abs(σ_px)
    p.sum_diffusion_y += abs(σ_py)           
    p.sum_diffusion_z += abs(σ_pz)
...
#     Heisenberg!(p.∇H_z, eiωt)
    ...
    update_momentum_operator!(p, 1) # mat_aux2 now is fx(t) (Heisenberg picutre force operator)
...
function update_momentum_sq_operator_old!(p, k)
    dt = p.dt
    Pk_sq = p.mat_aux
...
function plot_average_diffusion(results)
    max_t_id = 1
     plot_ts = Float64[]
    for i in 1:length(results.times)
         if length(results.times[i]) > max_t_id
