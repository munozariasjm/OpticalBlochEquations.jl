...
function evolve_with_diffusion!(p_diffusion, dψ, ψ, t, δt)
    ...
    if rand() < δp
        Δt = t - p_diffusion.t_lastdecay
        p_diffusion.Δt = Δt

        # give diffusion kick
        p_variance = calculate_force_variance(p_diffusion, ψ) * (Δt / 2)
        p_diffusion.D = p_variance
        ...
        p_diffusion.t_lastdecay = t

        # collapse wavefunction and give a recoil kick from spontaneous decay
        collapse!(ψ, p)
        recoil_kick(ψ, p, 1)
    end
