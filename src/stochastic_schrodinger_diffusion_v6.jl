...
function evolve_with_diffusion!(i, p_diffusion, ts, ψs, dψ, ψ, δt)
    ...
    if rand() < δp
        ...
        # fψ_to_χ!(p, ψ0, fϕ, a, k) # create |χ(0)⟩ = (a + bf)|ψ(t)⟩
        update_fields_for_diffusion!(p, ψt, t)
        fψ_to_χ!(p, ψt, fϕ, a, b, k)
        norm_fϕ = normalize!(p, fϕ)
        for idx ∈ t_idx:(-1):t0_idx
            τ = ts[idx]
            ψτ = ψs[idx]
            evolve_backwards!(p, dfϕ, fϕ, τ, δt)
            c_sum += force_stochastic_ψ1ψ2(p, ψτ, fϕ, 1) * norm_fϕ^2
        end

        # # forwards evolution
        # update_fields_for_diffusion!(p, ψ0, t0)
        # fψ_to_χ!(p, ψ0, fϕ, a, b, k)
        # norm_fϕ = normalize!(p, fϕ)
        ...
        #     ψτ = ψs[idx]
        #     evolve_no_jumps!(p, dfϕ, fϕ, τ, δt)
        #     c_sum += force_stochastic_ψ1ψ2(p, fϕ, fϕ, 1) * norm_fϕ^2
        #     # c_sum += force_stochastic_ψ1ψ2(p, ψτ, fϕ, 1) * norm_fϕ^2
        # end

        # a = -1
        # # update_fields_for_diffusion!(p, ψt, t)
        # # fψ_to_χ!(p, ψt, fϕ, a, k) # create |χ(0)⟩ = (1 + af)|ψ(t)⟩
        # update_fields_for_diffusion!(p, ψ0, t0)
        # fψ_to_χ!(p, ψ0, fϕ, a, k)
        # norm_fϕ = normalize!(p, fϕ)
        # for idx ∈ t0_idx:(t_idx-1)
        ...
        # a = im
        # # update_fields_for_diffusion!(p, ψt, t)
        # # fψ_to_χ!(p, ψ0, fϕ, a, k) # create |χ(0)⟩ = (1 + af)|ψ(t)⟩
        # update_fields_for_diffusion!(p, ψ0, t0)
        # fψ_to_χ!(p, ψ0, fϕ, a, k)
        # norm_fϕ = normalize!(p, fϕ)
        # for idx ∈ t0_idx:(t_idx-1)
        #     τ = ts[idx]
        #     evolve_no_jumps!(p, dfϕ, fϕ, τ, δt)
        #     c_sum -= im * force_stochastic_ψ1ψ2(p, fϕ, fϕ, 1) * norm_fϕ^2
        # end
        ...
        p_diffusion.D += real(c_sum) * δt #/ 4

        update_fields_for_diffusion!(p, ψt, t)
        p_diffusion.D -= p_diffusion.F_sum * force_stochastic_ψ1ψ2(p, ψt, ψt, 1) * δt
...
"""
    Evolve the wavefunction ψ from time t to time t + δt, where δt is a fixed timestep.
"""
function evolve_backwards!(p, dψ, ψ, t, δt)
    ...
    # evolve the state
    for i ∈ eachindex(ψ)
        ψ[i] -= dψ[i] * δt # note the negative sign
    end
...
function update_fk_matrix!(p, k, fk)
    ...
    @inbounds @fastmath for q ∈ 1:3
        ...
        for j ∈ eachindex(ds_q)
            ...
            ρ_mn = c_m * conj(c_n) #* conj(ψ1[m]) * ψ2[n]
